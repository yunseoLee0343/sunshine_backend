...
class ChunkRepository:
    ...
    async def get_document(self, plant_knowledge_id: uuid.UUID, chunk_kind: str) -> PlantChunkDocument | None:
    ...
    async def list_documents(self, plant_knowledge_id: uuid.UUID) -> list[PlantChunkDocument]:
        result = await self.session.execute(
            select(PlantChunkDocument).where(PlantChunkDocument.plant_knowledge_id == plant_knowledge_id)
        )
        return list(result.scalars().all())

    async def get_embedding(self, chunk_document_id: uuid.UUID) -> PlantChunkEmbedding | None:
